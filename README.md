...
启动流程：立即显示登录窗口，同时在后台线程同步用户信息与其他数据（拉取到本地 `data/`，`config.json` 作为远程配置叠加生效）并静默完成版本检查，不阻塞界面；登录必须等待同步成功（同步期间按钮禁用，失败则阻止登录并提示重试）→ 登录成功 → 如需更新则切换为更新界面，更新完成后自动重启并自动登录进入主窗口；`data.csv` / `rules.txt` 可通过 `--sync-push` 推回仓库。
